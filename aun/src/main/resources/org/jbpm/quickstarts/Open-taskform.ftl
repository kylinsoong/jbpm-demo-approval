<script type='text/javascript'>
	...
	function taskFormValidator() {
		...
		var myInputs = new Array();
					myInputs[i] = document.getElementById("_user");
					i++;


		var j=0;
						if(notEmpty(myInputs[j]) && !isAlphanumeric(myInputs[j])) {
							alert("Please enter valid _user");
							myInputs[j].focus();
							return false;
						}
		...
		<fieldset>
          ...
          <fieldset>
            <legend>Task Outputs</legend>
                            		<label for="name">_user</label>
                            		<div class="div_texbox">
                              		<input name="_user" type="text" class="textbox" id="_user" value="${_user}" />
                            		</div>
